...
parser.add_argument('--id', type=int, default=113 )
parser.add_argument('--learn_iter', type=int, default=1)
...
config['learn_for'] = 200 #offline instances
config['V_C'] = 50
...
def run_func(param = None):
    ...
    for n in range(config['learn_iter']):
        ...
        for k in range(config['runs']):
            ...
            sim.save_instance(train_folder + folder + '/Instance_' + str(n) + _ + str(k) + ".csv")
            ...
            while not end:
                ...
                if len(state['assignment']) < M and state['last_called'] != sim.N - 1:
                    policy_action = d.get_action(state)
                    #expert_action = d.get_expert_action(state)   
                    expert_action, avg_wait = d.get_avg_expert_action(state )
                    #call = d.add_new_data(state, policy_action, expert_action)
                    call = d.add_new_data_avg(state, policy_action, expert_action, avg_wait)
                    #d.add_features(k, n)
                else:
                    call = 0
...
def run_instance(n, k, sim, d, folder, ):
    print("Episode - ", k)
    state = sim.initialize(k, policy_name, rd_values = 'generate')   
    sim.save_instance( folder + '/Instance_' + str(n) + _ + str(k) + ".csv")
    if config['type'] == 'class':
        if config['target'] == 'called':
            d.pred_history = pd.DataFrame(columns=[ i for i in range(2 * max_calls)] + ['time', 'action', 'run'])
        else:
